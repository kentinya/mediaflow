...
class MetadataReviewBatchTests(unittest.TestCase):
    ...
    def test_batch_resolves_oldest_first_pending_reviews_atomically(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            with SQLiteTaskRepository(Path(directory, "runtime.sqlite3")) as repository:
                ...
                service = MetadataReviewService(repository)
                ...
                for review in reviews:
                    stored = repository.get_metadata_review(review.review_id)
                    self.assertEqual(stored.status, MetadataReviewStatus.RESOLVED)
                    self.assertEqual(stored.selected_rank, 1)
                    self.assertEqual(stored.selected_provider, "tmdb")
                    self.assertEqual(stored.selected_provider_id, "101")
                    self.assertEqual(stored.selected_media_type, "movie")
                    self.assertEqual(stored.actor, "operator")
                    self.assertEqual(
                        repository.get_item(review.item_id).status, TaskItemStatus.PENDING
                    )
                    audit = repository.list_metadata_review_audit(review.review_id)[0]
                    self.assertEqual(audit.selected_rank, 1)
                    self.assertEqual(audit.note, "reviewed")
                self.assertEqual(
                    repository.get_metadata_review(third[3].review_id).status,
    ...
    def test_task_filter_and_limit_select_only_selected_reviews(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            with SQLiteTaskRepository(Path(directory, "runtime.sqlite3")) as repository:
                coordinator = PersistentTaskCoordinator(repository, repository)
    ...
    def test_invalid_rank_actor_limit_empty_selection_and_missing_rank_are_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            with SQLiteTaskRepository(Path(directory, "runtime.sqlite3")) as repository:
                _, task, _, _ = self._waiting(repository)
                service = MetadataReviewService(repository)
                with self.assertRaisesRegex(ValueError, "rank"):
                    service.resolve_pending(True, actor="operator")
                with self.assertRaisesRegex(ValueError, "actor"):
                    service.resolve_pending(1, actor="   ")
                for limit in (0, -1, 101, True):
                    with self.subTest(limit=limit):
                        with self.assertRaisesRegex(ValueError, "limit"):
                            service.resolve_pending(1, actor="operator", limit=limit)
                with self.assertRaisesRegex(ValueError, "no pending"):
                    service.resolve_pending(1, actor="operator", task_id="missing")
                with self.assertRaisesRegex(ValueError, "rank"):
                    service.resolve_pending(99, actor="operator", limit=1)
    ...
    def test_any_invalid_member_rolls_back_whole_batch(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            connection.execute(
                """CREATE TRIGGER reject_metadata_review_batch
                BEFORE INSERT ON metadata_review_decision_audit
            ...
            with SQLiteTaskRepository(database) as repository:
                ...
                with self.assertRaises(sqlite3.IntegrityError):
                    service.resolve_pending(1, actor="operator", limit=2)
                self.assertEqual(
                    repository.get_metadata_review(first_review.review_id).status,
                    MetadataReviewStatus.PENDING,
                )
                self.assertEqual(
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            def resolve(actor):
                try:
                    with SQLiteTaskRepository(database) as repository:
                        barrier.wait()
                        MetadataReviewService(repository).resolve_pending(
                            1,
                            actor=actor,
                            limit=2,
                        )
            ...
            with SQLiteTaskRepository(database) as repository:
                self.assertEqual(
                    len(repository.list_metadata_review_audit(first_review.review_id)), 1
                )
                self.assertEqual(
                    len(repository.list_metadata_review_audit(second_review.review_id)), 1
                )

    def test_cli_batch_review_constructs_no_storage_or_provider(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            with patch.object(
                ...
                "create_storages",
                side_effect=AssertionError("batch metadata review constructed Storage"),
            ):
                code = final_main(
                    [
                        "--config",
            ...
            self.assertIn("BATCH METADATA REVIEW", output.getvalue())
    ...
    def test_c_preservation_and_retryable_selection_remain_unchanged(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            with SQLiteTaskRepository(Path(directory, "runtime.sqlite3")) as repository:
                ...
                MetadataReviewService(repository).resolve_pending(1, actor="operator", limit=1)
                ...
                self.assertEqual(repository.get_metadata_review(review.review_id).selected_rank, 1)
