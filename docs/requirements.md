...
| UX-006 | REQUIRED | Any possible media mutation is preceded by an equivalent Preview/DryRun and explicit execution authority that is separate from Preview. | §安全预览; §Dry Run 整理预演; §高风险文件操作保护; §V1 Scheduled Unattended Organization 授权 | Preview performs zero mutation and cannot itself grant authority; authority may be a one-shot manual decision or a valid, explicit, scoped and revocable Automation Task Definition grant. |
