...
| UX-006 | REQUIRED | Any possible media mutation is preceded by an equivalent Preview/DryRun and a separate explicit execution decision. | §安全预览; §Dry Run 整理预演; §高风险文件操作保护 | Preview performs zero mutation and cannot itself grant execution authority. |
...
| ID | Scope | Requirement | Canonical Source | Acceptance Meaning |
|---|---|---|---|---|
| REQ-META-001 | REQUIRED | Metadata is accessed through a provider abstraction; TMDB is the initial V1 provider and business code does not depend on TMDB DTOs or HTTP APIs. | §元数据服务; §元数据 Provider 抽象; §第一阶段 MVP / Metadata | Provider responses are converted into internal candidates and identities. |
...
| REQ-META-004 | REQUIRED | MetadataPolicy is independent from Provider and selects query type, locale, matching thresholds, search behavior, cache and permitted fallback/provider choices. | §元数据策略 | Multiple policies can reuse one Provider and a type can switch only to a configured allowed Provider. |
...
| REQ-SCHED-003 | REQUIRED | Schedule emission is durable, deterministic and idempotent and creates queued work rather than directly invoking media mutation. | Engineering derivation from §定时任务, §任务系统 and §安全预览 | Restart/concurrency cannot duplicate an occurrence or bypass Preview/execute policy. |
...
| REQ-CONFIG-008 | REQUIRED | Runtime, API, workers, Tasks and scheduled work resolve the same immutable Active or pinned snapshot identity and fail closed if unavailable or invalid. | Engineering derivation from §配置生命周期, §任务系统 and §定时任务 | UI Active state and actual engine behavior cannot diverge or fall back silently. |
...
| REQ-WEB-004 | REQUIRED | Web manages Storage, libraries, Metadata Providers, RecognitionTypes/policies, Tasks, manual identity correction and DryRun. | §第一阶段 MVP / UI; §配置管理 | Each management surface uses the same validation and safety contract as API. |
...
| Scheduled automatic organization | `REQ-SCHED-*`, `REQ-TASK-*`, `UX-006`, `REQ-SAFE-*` | A configured schedule emits bounded work that satisfies Preview, conflict and execution authority before mutation. |
