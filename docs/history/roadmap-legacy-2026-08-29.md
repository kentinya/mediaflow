# LEGACY / READ ONLY / NOT CURRENT WORKFLOW AUTHORITY

This is the complete pre-migration Roadmap snapshot from 2026-08-29. It preserves historical
Phase/Task/Fix/SHA/review evidence and must not be used to authorize current work. Current priority
and Slice status live in [`docs/roadmap.md`](../roadmap.md); current workflow lives in
...
| Phase 22.6-M One Provable Per-Sample Destination Row Shape | PASS / CLOSED | `03e64d40023753be13b2cce18b8c5a63492d344a` | PASS — 2026-08-29 独立审核（BASE `32e6e76f348c5c10d08ca247eaf01112aa109f0c`）：scope 精确到三个文件——`mediaflow/application/configuration_objects.py` +2/-0、`tests/test_configuration_destination_precheck.py` +165/-1 与 `TASK.md` +83/-10；生产 numstat 恰为 `2	0`，`git diff --exit-code 32e6e76 HEAD -- mediaflow/domain mediaflow/infrastructure mediaflow/interfaces mediaflow/cli.py scripts config pyproject.toml docs` 为空，故 Web 面板、两份 CURRENT 文档、API、CLI、打包元数据与两个 marker（10 / 22）逐字节未动，另两个测试模块未开。生产改动就是被要求的两行：单样本完成行（`:1779`）与 `_probe_destination_sample` 行（`:2201`）各在 `"message": None,` 之后补 `"nextAction": None,`，零删除；我对该文件在 BASE 与 HEAD 各做 AST 审计，调用名集合完全相同（新增 `[]`、删除 `[]`），未引入任何调用、分支或写操作。十键形状claim 是完备的而非抽样：所有 `result.items[]` 行只出自四处（单样本内联 `:1768-1780`、`_probe_destination_sample`、`_destination_sample_failure_row` `:2231-2244`、`_destination_sample_resolution_row` `:2246-2259`），装配在 `:2051`、由 `_destination_multi_result`（`:2265-2274`）包装，`"items":` 仅出现在 `:1768`、`:2051`、`:2272`；我自己的离线运行（单样本、八样本全成功、八样本全失败、碰撞、多 Storage）均报 `distinct row shapes = 1`，键序恰为 `index` 至 `nextAction` 十项。测试与 Task 逐条吻合：唯一授权删除是 `:648` 的 `.get("nextAction")` 改直接下标（`grep 'get("nextAction")'` now 无匹配）、resolution 行五条下标断言在 `:785` 之后、单样本断言在 `:810` 之后，新增 `test_destination_precheck_rows_share_one_key_shape_across_branches`（`:821-977`）以三次离线 `tempfile.TemporaryDirectory()` 运行逐行断言 `tuple(row.keys())`，并两侧钉住 `nextAction`（一失败行为 `str`、一成功行为 `None`）与分支身份；模块 21 → 22，回归 872 `OK (skipped=7)`、聚焦 58（29+22+7）。我自己跑十四项探针加 control（逐次一处改动、`git checkout --` 还原、核对干净树）：删单样本新键 → 形状测试 FAIL 且单样本测试 `KeyError` ERROR；删探测行新键 → 形状测试与 `..._carry_their_own_next_action` 双失败；删 resolution 行 `"message": None` → 形状测试与 `test_multiple_destination_storages_is_bounded_failure` 双失败，正是 22.6-L 上静默通过的那处变异，该观察闭合；resolution 行 `targetExists` 改 `True` 或 `nextAction` 改默认句子 → 命名测试 FAIL；多余 `debugNote` 键与三处键序互换（resolution、探测、单样本）→ 形状测试 FAIL；两处内联行改 `""` → 各自的测试 FAIL（证明钉的是值而非仅键存在）；删失败行 `message` → 五项失败、删其 `nextAction` → 两项失败（既有证明未被挤掉）；comment-only control `Ran 22 tests ... OK`；无一项静默通过，且这些探针共同证明那一项形状测试确实观察到四个 builder。闸门全绿：ruff/format 308 文件、compileall、`pip check`、两份示例配置校验、wheel 隔离安装 smoke 退出 0 且 `Supported schema: 22`/`Runtime schema: 22`/`Schema: 22`、`git diff --check`、FFmpeg 审计无匹配、业务层写操作 AST 审计只剩四处既有 Storage 接口调用、alist ignore（0 tracked / 0 staged）、Markdown 120 文件 25 链接 0 断链、机密扫描仅命中 `TASK.md` 中描述闸门的 “secret”/“alist” 字样。体量证据独立复现：域实体以 `json.dumps(value, ensure_ascii=False, separators=(",", ":"))` 对 `CONFIGURATION_STRATEGY_RESULT_LIMIT` 设卡（`mediaflow/domain/configuration_management.py:596-598`），报告的 `11049` 字节在八样本 200 字符 ASCII 标题、全部 `completed` 时逐字复现；新键在八个 `null` 行上共 144 字节、八行全失败时共 704 字节（最长动作串 74 字节，解析上界 720）；我能构造的饱和最坏值为 25,035 字节（八个长 CJK 标题，目标字段在 408 字符处停止增长），为 32,768 上限的 76%。非阻塞观察（不得升格为 blocker）：报告称 Markdown 审计 125 文件，仓库实为 120 个受版本控制的 `.md`，`find . -name '*.md'` 含 `.venv/` 恰为 125，即多计五个依赖自带文件，链接数 25 与 0 断链无误；我自己的 22.6-M Task 估“每行约 22 字节”偏松，实测 `null` 行 18 字节、失败行至多 90 字节，结论不变；形状测试用 `assertGreaterEqual(len(rows), 6)` 而 fixture 恰为 6，是下限而非精确计数（并不空洞：每行单独断言且全部探针命中）；目标结果没有识别路径那样的截断阶梯（`configuration_objects.py:3382-3423`），上限以抛 `ValueError` 落实，在 76% 余量下属既有性质；`docs/architecture.md:1158-1160` 的 CURRENT 说法现在对四个 builder 都字面成立（此前略微超前），故文档逐字节不变是正确选择，只是同序十键形状本身仍未写入文档。新测出的残留缺口一项（即下一 Slice）：把多样本 verdict 表达式（`:2043-2045`）整体换成常量 `"manual_confirmation_required"` 后全套 872 项仍 `OK`——因为没有任何“全部样本 `ready`”的多样本运行断言 `verdict == "ready"`，也没有任何多样本运行断言 `capability_gap` 覆盖；而 `min` 与常量 `"ready"` 两个方向均被三项既有测试捕获。我已只读验证两个缺失方向都可离线达成：三个互异 ready 样本得 `verdict == "ready"`，两样本 HARD_LINK 且 `_can_hard_link` 打桩为 `False` 得 `verdict == "capability_gap"` 与 `missingStorageCapabilities == ["can_hard_link"]`，而每行 `projectedOutcome` 仍为 `ready`。22.6-H-F1 与 22.6-I 的其余残留缺口未变。Phase 22.6-M 由此 **PASS / CLOSED**，Phase 22.6 保持开放；未推送；下一合法 Slice 为 **Phase 22.6-N**（两侧证明多样本 verdict：两项新增证据测试、零生产改动、Web 与文档逐字节不变） |
| Phase 22.6-N Prove The Multi-Sample Verdict In Both Directions | PASS / CLOSED | `5884905c2105cf8ff78ff10d1b872875045769d7` | PASS — 2026-08-29 独立审核（BASE `dbde27979ed964614a6cfd51ad4f9338fce652f4`）：checkpoint 仅改 `TASK.md` 与 `tests/test_configuration_destination_precheck.py`（+116/-0），生产、Web/API、配置、脚本、schema 与产品文档逐字节未动；测试模块 22 → 24，只新增 TASK 点名的 all-ready 与 HardLink capability-gap 两项离线只读证明，均使用直接下标、独立临时根、`finally` 关闭仓库、零 mutation/authority 且目标树不变。独立把 verdict 固定为 `manual_confirmation_required` 时恰好两项新测试失败，去掉 capability guard 时恰好 capability-gap 新测试失败；恢复后生产 blob 与 checkpoint 相同。聚焦 60、全量 874（7 skips）、Ruff/format/compileall/`pip check`、两份配置校验、wheel 隔离 smoke、marker 10/22、Markdown 120/25/0、FFmpeg/业务写边界/alist/机密审计全部通过。BASE TASK 的 Python fence 自身不符合 Ruff Markdown 格式，checkpoint 的等价单行格式化为机械修正，不构成 scope 扩张。Phase 22.6-N CLOSED；Phase 22.6 仍开放。下一合法 Slice 是 Phase 22.6-O（仅同步已陈旧的 CURRENT 文档陈述），其 PASS 后才允许 phase-level Final Closure Audit；六项移出能力保持后置。 |
...
2026-08-29 独立审核 Phase 22.6-M checkpoint `03e64d40023753be13b2cce18b8c5a63492d344a` 判定 **PASS**，
Phase 22.6-M **PASS / CLOSED**。scope 精确：三个文件——`configuration_objects.py` +2/-0、
`tests/test_configuration_destination_precheck.py` +165/-1、`TASK.md` +83/-10；BASE
`32e6e76` 相对的 `mediaflow/domain`、`mediaflow/infrastructure`、`mediaflow/interfaces`、
`mediaflow/cli.py`、`scripts`、`config`、`pyproject.toml` 与 `docs` 全为空，故 Web、两份 CURRENT 文档、
API、CLI 与两个 marker（10 / 22）逐字节未动。生产改动就是被要求的两行（`:1779` 与 `:2201` 各补
`"nextAction": None,`，零删除），BASE 与 HEAD 的 AST 调用名集合完全相同，未引入任何调用或写操作。
十键形状完备：所有行只出自四个 builder，装配点与 `"items":` 站点已逐一枚举，我自己的五种离线运行均报
`distinct row shapes = 1`。测试仅一处授权删除（`:648` 的 `.get` 改下标），新增一项跨分支形状测试逐行断言
键序并两侧钉住 `nextAction`；模块 21 → 22，回归 872 `OK (skipped=7)`、聚焦 58。我跑十四项探针加 control：
删两处新键、删 resolution 行 `"message"`、改 `targetExists`/`nextAction` 值、加多余键、三处键序互换、
两处改 `""`、删失败行两键——全部只让相应测试失败，comment-only control 22 项全绿，22.6-L 记录的
`"message": None` 静默缺口由此闭合。闸门全绿（ruff/format/compileall/`pip check`/两份示例配置/wheel 隔离
smoke 退出 0 且 `Schema: 22`/`git diff --check`/FFmpeg 与业务层写操作审计/alist ignore/Markdown 0 断链/
机密扫描）。体量：域实体的 `ensure_ascii=False` 紧凑编码即上限口径，报告 `11049` 字节逐字复现，新键计
144 字节（八个 `null` 行）至 704 字节（八行全失败），饱和最坏 25,035 字节为上限的 76%。非阻塞观察：
报告的 Markdown “125 文件”含 `.venv/` 的五个文件（仓库实为 120，链接 25、0 断链无误）；我自己 Task 的
“每行约 22 字节”偏松（实测 18 / 至多 90）；形状测试用下限而非精确行数；目标结果无截断阶梯而以抛错设卡；
CURRENT 文档说法现已字面成立但未记载同序十键。新测出的残留缺口即下一 Slice：多样本 verdict 表达式
（`:2043-2045`）整体换成常量 `"manual_confirmation_required"` 后全套 872 项仍 `OK`，`capability_gap`
覆盖同样无多样本证明。下一合法 Slice 是 **Phase 22.6-N**（两侧证明多样本 verdict：一项“全部样本 ready →
`verdict == "ready"`”测试与一项“能力缺口覆盖 ready 行”测试，零生产改动，Web 与文档逐字节不变），
已写入 `TASK.md`。Phase 22.6 保持开放；本轮 checkpoint 与审核记录均未推送；移出 Phase 22.6 的
六项能力维持后置。

2026-08-29 独立审核 Phase 22.6-N checkpoint
`5884905c2105cf8ff78ff10d1b872875045769d7` 判定 **PASS / CLOSED**。该 Slice 只增加两项
多样本 verdict 证据测试，生产、Web/API、配置、脚本、schema 与产品文档逐字节未动；全量
874 项（7 skips）与聚焦 60 项通过，两个核心独立 mutation 分别被新测试捕获。Phase 22.6
仍未关闭。审核发现数处早于 N 的 CURRENT 文档状态陈述已经陈旧，因此下一合法 Slice 是
**Phase 22.6-O — Reconcile Phase 22.6 CURRENT Documentation Before Final Closure**：只允许同步
`docs/requirements.md`、`docs/product-experience.md`、`docs/architecture.md` 与 `TASK.md`，零生产、
测试、schema、Roadmap/Progress 行为改动。22.6-O 通过独立 High 后才允许 phase-level Final Closure
Audit；六项已移出能力继续后置，不得开始 Phase 23。
...
- Phase 22.6 起始 Slice 22.6-A（managed NamingPolicy CRUD + 精确 revision 离线命名预览）已
  ...
  **PASS / CLOSED**：22.6-I（run 级摘要与首样本分离，`6c0ba745772e315b941c1c3b314ab47e66e8f35a`）、
  ...
  证据修正接受；被拒 checkpoint `74919a33ac5ec9cde5b104a591ef3fdfb25a1bf3` 与其记录保留）；22.6-M
  （统一逐样本目标行形状并加以证明：两处内联行各加 `"nextAction": None`，一项跨分支形状测试钉住同序
  十键）亦已 **PASS / CLOSED**（`03e64d40023753be13b2cce18b8c5a63492d344a`）；22.6-N（两侧证明
  多样本 verdict：全部样本 `ready` 时 `verdict == "ready"`，能力缺口覆盖 ready 行，零生产改动）
  亦已 **PASS / CLOSED**（`5884905c2105cf8ff78ff10d1b872875045769d7`）。当前下一 Slice 是
  22.6-O，仅同步 Phase 22.6 CURRENT 文档状态与已实现语义；仍为只读、零生产/测试/schema 变更，
  且不得宣告 Phase 22.6 关闭。22.6-O 独立 PASS 后才允许 Final Closure Audit。
...
下一正式 Task 是 **Phase 22.6-O — Reconcile Phase 22.6 CURRENT Documentation Before Final
Closure**；Phase 22.6-A 至 22.6-N 已全部通过独立 High Review 判定 **PASS / CLOSED**（22.6-E、22.6-G、
...
于 2026-08-29 判 **FIX REQUIRED** 并保留不改写；22.6-M 的接受 checkpoint 为
`03e64d40023753be13b2cce18b8c5a63492d344a`，22.6-N 的接受 checkpoint 为
`5884905c2105cf8ff78ff10d1b872875045769d7`）。22.6-A 至
22.6-K 的全部 checkpoint 与审核记录已推送 `origin/main`（2026-08-29 第二次授权推送，`main` 与 `origin/main`
当时同为 `af9ca9a`；此后 `74919a3`、我的 22.6-L 审核记录 `cf99c6b`、22.6-L-F1 接受 checkpoint
`b198c96`、22.6-L PASS 记录与 22.6-M TASK 定义 `32e6e76`、22.6-M 接受 checkpoint `03e64d4`、
22.6-M PASS / 22.6-N Task 记录 `dbde279`、22.6-N 接受 checkpoint `5884905` 与本轮审核记录尚未
推送，需新的显式授权），Phase 22.6 的 phase-level Final Closure Audit 仍未执行，且必须等待
22.6-O 独立 PASS；推送本身不构成 Phase 关闭。不得借此提前开展 Provider
