...
#: Most file items one bounded Upload request may carry.  The count is enforced
#: at admission, before the first byte is streamed, so an over-limit request is
#: refused with zero mutation.
MAX_UPLOAD_ITEMS = 512

#: Most directory items one bounded Upload request may create.
MAX_UPLOAD_DIRECTORIES = 512

#: Deepest directory nesting one bounded Upload may create.
MAX_UPLOAD_DEPTH = 32

#: Largest single file one bounded Upload may carry.
MAX_UPLOAD_ITEM_BYTES = 512 * 1024**2

#: Largest total byte size one bounded Upload request may cover.
MAX_UPLOAD_BYTES = 10 * 1024**3

#: Largest item for which one durable Upload result records a streaming SHA-256
#: checksum.  Bigger items record size and verified effects only, so the
#: durable evidence stays bounded.
MAX_UPLOAD_CHECKSUM_BYTES = 64 * 1024**2

#: Largest bounded Upload manifest (the item list ahead of the byte payloads)
#: one Upload request may carry.  The framing header is read in full; the
#: payloads are streamed, so only the manifest is ever buffered.
MAX_UPLOAD_MANIFEST_BYTES = 512 * 1024

#: Most top-level paths one bounded Download selection may name.
MAX_DOWNLOAD_PATHS = 50

#: Most entries one bounded Download archive enumeration may visit before it
#: refuses to continue.  The same bounded scope protects admission and every
#: archive entry.
MAX_DOWNLOAD_ENTRIES = 5000

#: Deepest directory nesting one bounded Download enumeration may descend.
MAX_DOWNLOAD_DEPTH = 32

#: Largest total byte size one bounded Download (single-file stream or archive)
#: may cover.
...
class UploadItemStatus(StrEnum):
    """The per-item outcome of one bounded Upload selection.

    Independent per-item outcomes keep a partially uploaded selection
    recoverable: a conflicting, failed or uncertain item never hides,
    overwrites or blocks the diagnosis and safe retry of its siblings.
    ``UNCERTAIN`` marks one item whose write crossed the mutation boundary
    but could not be proven complete; it is never replayed automatically.
    Replace/overwrite is deliberately absent; the conflict choices are the
    Transfer conflict semantics (``UploadConflictChoice``).
    """
...
class UploadConflictChoice(StrEnum):
    ...
    ``NO_OVERWRITE`` is the default: a conflicting destination fails that item
    only and nothing is replaced.  ``SKIP`` leaves a conflicting destination
    untouched and records the item as skipped.  ``KEEP_BOTH`` publishes the
    upload at a backend-generated unique name.  Replace is intentionally
    absent: it would require one destination-bound destructive confirmation.
    """
...
class UploadItemPlan:
    """One bounded, validated upload item, confined to one library-relative tree.

    ``relative_path`` is the exact destination relative to the selected
    ResourceLibrary directory, built from safe browser-supplied relative
    paths.  ``size`` is the operator-declared byte count the request must
    deliver for this item; the backend re-proves it while streaming and
    records a bounded checksum on the durable result.  ``is_directory`` marks
    an empty-directory placeholder that is created with no byte payload.  No
    host path, credential or browser-internal detail crosses into the
    manifest.
    """
...
class UploadManifest:
    """The pinned, confirmed bounded scope of one Upload request.

    The manifest binds the exact Active configuration revision/digest, the
    destination ResourceLibrary/Storage identities, the confined destination
    directory, the explicit conflict choice, the ordered per-item plan and its
    opaque digest.  Only the digest is returned to the browser, mirroring the
    Copy/Move manifest model.
    """
...
class UploadItemOutcome:
    """The bounded outcome of one uploaded item.

    ``destination`` is the exact confined path the item was published at
    (a backend-generated keep-both name when a rename happened), or the
    pinned admission destination when nothing was written.  ``error_category``
    carries a stable, secret-free reason for a non-success item; it never
    exposes raw exception text, provider DTOs or host paths.
    """
...
@dataclass(frozen=True)
class UploadResult:
    """The aggregate bounded outcome of one confirmed Upload command.

    Every item keeps its independent outcome; the aggregate status follows the
    same precedence used by transfers.  The durable Task identity lets the Web
    surface follow the result without raw implementation ceremony.
    """

    manifest_digest: str
    conflict: UploadConflictChoice
    outcomes: tuple[UploadItemOutcome, ...]
    total_items: int
    succeeded_items: int
    skipped_items: int
    failed_items: int
    ...
    task_id: str
    task_status: str
    #: Bounded per-item diagnostic window; ``None`` when every item fit.
    max_outcomes: int = MAX_TRANSFER_PROGRESS_ENTRIES

    def document(self) -> dict[str, object]:
        shown = self.outcomes[: self.max_outcomes]
        return {
            ...
            "outcomesTruncated": len(self.outcomes) > self.max_outcomes,
...
class DownloadItemStatus(StrEnum):
    """The per-item outcome of one bounded Download selection.

    Download is a confined zero-mutation read: an item that disappears or
    changes mid-stream is recorded honestly in the response trail; it is never
    fabricated, and an interrupted download remains safe to repeat.
    """
...
class DownloadArchiveEntry:
    """One bounded entry of one confined Download archive manifest.

    The entry pins the archive-internal path, the exact source entry observed
    at enumeration time and its byte size, so the streaming writer publishes
    exactly the confirmed scope.  When an observed entry can no longer be read,
    the manifest records the honest ``DownloadItemStatus`` outcome instead of
    fabricating content.
    """
...
class DownloadManifest:
    """The bounded, pinned read scope of one Download request.

    Exactly one of ``single_file`` (direct stream) and ``entries`` (one
    generated archive) is set.  The archive manifest is generated on the fly
    and never written back into managed Storage.  No credential, host path or
    provider detail appears here.
    """

    resource_library_id: str
    filename: str
    single_file: DownloadArchiveEntry | None = None
    entries: tuple[DownloadArchiveEntry, ...] = ()
    manifest_note: str | None = None

    @property
    def is_archive(self) -> bool:
        return self.single_file is None

    @property
    def total_bytes(self) -> int:
        if self.single_file is not None:
            return self.single_file.size
